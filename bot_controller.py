...
class SheinStockMonitor:
    ...
    def run_async(self, coro):
        """Run coroutine safely from sync code with or without active event loop."""
        try:
    ...
    def setup_database(self):
        ...
        db_path = self.config.get('database_path', 'shein_monitor.db')

        # /tmp is valid on Linux cloud runtimes but breaks on Windows.
    ...
    def check_stock(self, manual_check=False, chat_id=None):
        ...
        if current_stock == 0 and men_count == 0:
            ...
            if manual_check and chat_id:
                self.run_async(self.send_telegram_message(error_msg, chat_id))
            return
...
🔗 {self.config['api_url']}
            """.strip()
            self.run_async(self.send_telegram_message(status_message, chat_id))
        ...
        if men_stock_increased:
            ...
            self.run_async(self.send_men_stock_alert_to_all(men_count, prev_men_count, men_change))
            ...
            self.run_async(self.send_women_stock_alert_to_all(women_count, prev_women_count, women_change))
...
🎉 Everything is set up properly!
    ...
    def start_monitoring(self):
        ...
        self.run_async(self.send_test_notification())
        self.check_stock()
                ...
                if data.get('ok') and data.get('result'):
                    for update in data['result']:
                        ...
                        if 'message' in update and 'text' in update['message']:
                            ...
                            monitor.run_async(monitor.handle_telegram_command(text, chat_id, user_id))
                else:
